function Isosurfaces( volume, isovalue, screen )
{
  ...
  */
    ...
    for ( var i = 0; i < 256; i++ )
    {
        ...
        if (i == isovalue){
          var color_iso = new THREE.Color(R,G,B);
        }
    }
    //面に等値面の値を設定
    ...
    material .color = new THREE.Color(color_iso);
    //オブジェクトの色にwhiteを設定
    ...
    function interpolated_vertex( v0, v1, s )
    {
        ...
        //s0とs1の大小関係によって場合分け
        if ( s1 >= s0 ){
        var x = ( s - s0 ) * ( v1.x - v0.x ) / ( s1 - s0 ) + v0.x;
        var y = ( s - s0 ) * ( v1.y - v0.y ) / ( s1 - s0 ) + v0.y;
        var z = ( s - s0 ) * ( v1.z - v0.z ) / ( s1 - s0 ) + v0.z;
      }
      else {
        var x = ( s - s1 ) * ( v0.x - v1.x ) / ( s0 - s1 ) + v1.x;
        var y = ( s - s1 ) * ( v0.y - v1.y ) / ( s0 - s1 ) + v1.y;
        var z = ( s - s1 ) * ( v0.z - v1.z ) / ( s0 - s1 ) + v1.z;
      }
